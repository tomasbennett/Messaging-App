...
export async function addFriend(friendId: string, nav: NavigateFunction): Promise<IJWTFetchResponses<{ successfulFriendReqSentToId: string }> | null> {
    try {
        ...
        if (response.data.status === 201) {
            return {
                returnType: "response",
                data: {
                    successfulFriendReqSentToId: friendId
                }
            };
        }
